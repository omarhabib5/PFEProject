...
namespace Projet.Tests
{
    public class CreateSprintHandlerTests
    {
        ...
        [Fact]
        public async System.Threading.Tasks.Task CreateSprint_Throws_WhenNameAlreadyExistsInSameProject()
        {
            var options = new DbContextOptionsBuilder<ApplicationDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
            ...
            var action = async () => await handler.Handle(cmd, CancellationToken.None);

            await action.Should().ThrowAsync<InvalidOperationException>()
                .WithMessage("A sprint with this name already existed.");
        }
    }
}
